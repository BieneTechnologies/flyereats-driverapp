...
ons.ready(function() {
    ...
    kNavigator.pushPage("pageGetSettings.html", {});
    // showPage("pageGetSettings.html", 2);
    // navigator.resetToPage("pageGetSettings.html")
...
document.addEventListener("show", function(event) {
    ...
    switch (event.target.id) {
        ...
        case "home":
            var onDutyObj = document.getElementById('onduty');
            if (onDutyObj == null || typeof onDutyObj == 'undefined') {
            }
            else {

                var onduty = onDutyObj.checked == true ? 1 : 2;
            if (onduty == 1) {
...
document.addEventListener("init", function(event) {
    ...
    switch (event.target.id) {
        ...
        case "pageGetSettings":
            ...
            document.addEventListener("deviceready", function() {
            showAccessDisclosureOnce().then(function() {
                if (isDebug()) {
                    if (hasConnection()) {
                        ...
                        if (device_platform.toLowerCase() == 'android' && phones.indexOf(device.manufacturer.toLowerCase()) != -1) {
                            var autostart_access = getStorage("autostart_access");
                            if (autostart_access != 1) {
                                setStorage("autostart_access", 1);
                                try {
                                showAutoStartAccesDialog();
                            }
                                catch (err) {
                                    console.log(err);
                                }
            ...
            var calendar = $('#calendar').fullCalendar({
                ...
                dayClick: function(date, jsEvent, view) {
                    if (IsDateHasEvent(date)) {
                        displayOrderSummaryDialog(date);
                    }
                    // var event = calendar.getEventById(date.format());
...
/*mycall*/
function callAjax(action, params, isnotloader) {
    dump("action=>" + action);
                ...
                switch (action) {
                    case "login":
                        ...
                        try {
                            ...
                            // showPage("home.html", 2);
                            kNavigator.resetToPage("./views/home.html", {
                            animation: 'slide',
                        });
                        } catch (err) {
                            console.log(err);
                    ...
                    case "getTaskByDate":
                        ...
                        if (!empty(data.details.data)) {
                            if (data.details.data.length > 0 && (data.details.data[0].status == "assigned" || data.details.data[0].status == "unassigned" || data.details.data[0].status == "")) {
                                playSoundUntilTaskAcceptOrDecline();
                            }
                        ...
                        break;
                    case 'GetOrderSummaryByDate':
                        var ordSumDialog = document.getElementById('orderSummaryByDateDialog');
                        var displayDate = getStorage('kr_display_date_raw');
                        if (ordSumDialog) {
                            ordSumDialog.show();
                            $("#drvordsumtitle").html(displayDate);
                            $('#orderSummaryByDateDialog .valord').html(data.details.total_orders);
                            $('#orderSummaryByDateDialog .valcod').html(data.details.cod_orders);
                            $('#orderSummaryByDateDialog .valamount').html(data.details.cod_orders_amount);
                            if (data.details.total_distance == null || data.details.total_distance == 0) {
                                $('#orderSummaryByDateDialog .valdistance').html("NA");
                            }
                            else {
                                $('#orderSummaryByDateDialog .valdistance').html(data.details.total_distance);
                            }
                        } else {
                            ons.createElement('orderSummaryByDate.html', { append: true }).then(function(dialogord) {
                                dialogord.show();
                                $("#drvordsumtitle").html(displayDate);
                                $('#orderSummaryByDateDialog .valord').html(data.details.total_orders);
                                $('#orderSummaryByDateDialog .valcod').html(data.details.cod_orders);
                    ...
                    case "TaskDetails":
                        ...
                        stopSoundWhenTaskAcceptOrDecline();
                    ...
                    case "GetProfile":
                        ...
                        $(".push_device_id").val(getStorage("device_id"));
                        ...
                        setTimeout(function() {
                            ...
                            fillTransportList(data.details.transport_list, "");
                    ...
                    case "GetTransport":
                        ...
                        fillTransportList(data.details.list);
                        break;
                    ...
                    case "GetAppSettings":
                        ...
                        if (auto_login == 1) {
                            ...
                            kNavigator.resetToPage("./views/pagelogin.html", {
                                animation: 'fade',
                                callback: function() {
                                    var kr_username = getStorage("kr_username");
                                    var kr_password = getStorage("kr_password");
...
function toastMsg(message) {
    message = message || "A default message. ";
    if (isDebug()) {
        onsenAlert(message);
        return;
...
function reloadHome() {
    dump('reloadHome');
    if (reload_home == 1) {
        dump('getTodayTask : line no 1537')
        getTodayTask('');
        reload_home = 2;
    }
}
...
//Should not be used this function. use instead openTaskMap
...
//Replaced viewTaskMap to openTaskMap
function openTaskMap(task_id, task_lat, task_lng, delivery_address) {

    $("#task_lat").val(task_lat);
    $("#task_lng").val(task_lng);
...
function playNotification() {
    ...
    var sound_url = cordova.file.applicationDirectory + "www/beep.wav";
    dump(sound_url);
    if (!empty(sound_url)) {
        playAudio(sound_url);
    }
...
function playAudio(url) {
    ...
    my_media = new Media(url,
        ...
        // error callback
        function(err) {
            dump("playAudio():Audio Error: " + err.code + err.message);
        },
        function(status) {
...
function uploadPhoto(imageURI) {
    try {
        ...
        $(".profile-bg").css('background-image', 'url(' + imageURI + ')');
        $(".profile-bg").css("background-size", "cover");
        setStorage('camera_on', 2);
        ...
        ft.upload(imageURI, ajax_url + "/UploadProfile", function(result) {
            ...
            if (result.responseCode != 200) {
                ...
                toastMsg("Image upload failed, Please try again");
                return;
            }

            var response = explode("|", result.response);

            var r2 = JSON.parse(response[0].substring(1, response[0].length - 1));
            //alert(response[0]);
            toastMsg(r2.msg);

            if (r2.code == "1" || r2.code == 1) {
                $(".profile-bg").css('background-image', 'url(' + r2.details.profile_file + ')');
                $(".profile-bg").css("background-size", "cover");
                $(".avatar").attr("src", r2.details.profile_file);
...
playSound = function() {
    try {
        url = cordova.file.applicationDirectory + 'www/beep.wav';
    ...
    switch (push_actions) {
        ...
        case "UPDATE_TASK":
            ...
            if (current_page == "home") {
                setTimeout(function() {
                    dump('getTodayTask : line no 3216');
                    getTodayTask('');
                }, 1000);
            } else {
                reload_home = 1;
...
pushUnregister = function() {
    dump("pushUnregister");
    try {
        if (typeof push == "undefined" || push == null) return;
...
viewDropOffMap = function() {
    ...
    $("#task_lat").val(dropoff_lat);
    $("#task_lng").val(dropoff_lng);
    ...
    switch (event.index) {
        ...
        case 1:
            if (current_page == "home") {
                ...
                // callAjax("getTaskCompleted","date="+raw_date);
...
document.addEventListener('preshow', function(event) {
    ...
    switch (page_id) {
        case "dialogChangePass":
            ...
            break;
        case "orderSummaryByDateDialog":
            //            var rdate=getStorage('kr_todays_date_raw');
            //            var params = "date="+ rdate;
            //            callAjax("GetOrderSummaryByDate", params, true);
            break;
    }

});
function showAutoStartAccesDialog() {
    dump('showAutoStartAccesDialog');
    if (autostartdlg) {
        autostartdlg.show();
    } else {
        var autostdlg = document.getElementById('AutoStartAccesDialog');
        ons.createDialog('AutoStartAccesDialog.html').then(function(dialog) {
            autostartdlg = dialog;
            dialog.show();
        });
    }
}
function openPermissionApp() {
    dump('openPermissionApp');
    if (autostartdlg) {
        autostartdlg.hide();
    }
    AutostartPermission.openAutostartPermissionPopup();
}
function cancelPermissionApp() {
    dump('cancelPermissionApp');
    if (autostartdlg) {
        autostartdlg.hide();
    }
}

function displayOrderSummaryDialog(dtval) {
    var rdate = dtval.format();
    setStorage('kr_todays_date_raw', rdate);
    setStorage('kr_display_date_raw', dtval.format('MMM, DD'));
    var params = "date=" + rdate;
    callAjax("GetOrderSummaryByDate", params);

}
function closeOrderSummaryDialog() {
    var ordSumDialog = document.getElementById('orderSummaryByDateDialog');
    ordSumDialog.hide();
}
function viewmyorderslist() {
    dump('viewmyorders');
    closeOrderSummaryDialog();
    kNavigator.popPage().then(function() {
        //         setStorage('kr_todays_date_raw', date.format() );
        var dtvltitle = getStorage('kr_display_date_raw');
        var rdate = getStorage('kr_todays_date_raw');
        $(".todays_date").html(dtvltitle);
        document.querySelector('ons-tabbar').setActiveTab(1);
        dump('getTaskCompleted : line no 3931');
        callAjax("getTaskCompleted", "date=" + rdate + "&task_type=completed");
        // callAjax("getTaskCompleted","date="+rdate );
        //getTodayTask('');
    });
}
...
function showAccessDisclosureOnce() {

    disclouserPromise = $.Deferred();

    //This MUST be same once new build submitted to PLAYSTORE/APPSTORE
    var current_build = "2_3";
    var disclosure_key = "disclouser_accepted_" + current_build;
    var disclouser_accepted = getStorage(disclosure_key);

    if (disclouser_accepted == 1)
        disclouserPromise.resolve();

    else {
        ons.notification.confirm({
            message: 'FLYER Eats: DRIVER APP collects location data to send orders to drivers based on their current location even when the app is closed or not in use.',
            ...
            callback: function(index) {
                // 1: Reject
                if (index == 1) {
                    setStorage(disclosure_key, 0);
                    disclouserPromise.reject();
                }
                // 0-: Button index from the left
                else {
                    setStorage(disclosure_key, 1);
                    disclouserPromise.resolve();
                }
            }
        });
    ...
    return disclouserPromise.promise();

}
/****** PLAY SOUND UNTIL TASK ACCEPT/DECLINE ***/

var taskAcceptDeclineSound;
var taskAcceptDeclineSoundFlag;
var taskAcceptDeclineSoundDefaultFlag = { loop: true }; //loop: true, false
function playSoundUntilTaskAcceptOrDecline() {
    try {
        taskAcceptDeclineSoundFlag = Object.assign({}, taskAcceptDeclineSoundDefaultFlag);
        url = cordova.file.applicationDirectory + 'www/beep.wav';
        if (device_platform == "iOS") {
            url = "beep.wav";
        }
        //alert(url);
        taskAcceptDeclineSound = new Media(url,
            function() {
                dump("playAudio():Audio Success");

                if (taskAcceptDeclineSoundFlag.loop == false) {
                    taskAcceptDeclineSound.stop();
                    taskAcceptDeclineSound.release();
                }
                else {
                    taskAcceptDeclineSound.seekTo(0);
                    taskAcceptDeclineSound.play();
                }
        ...
        taskAcceptDeclineSound.play({ playAudioWhenScreenIsLocked: true });
        taskAcceptDeclineSound.setVolume('1.0');
...
function stopSoundWhenTaskAcceptOrDecline() {
    if (typeof taskAcceptDeclineSoundFlag == 'undefined') return;

    taskAcceptDeclineSoundFlag.loop = false;
...
/***** END PLAY SOUND UNTIL TASK ACCEPT/DECLINE ***/
